...
export default function Home() {
    ...
    const handleProcessCalculations = () => {
        ...
        if (Math.abs(kgPerWeek) > maxSafeKgPerWeek) {
            ...
            setErrorTracker(`⚠️ Obiettivo troppo aggressivo. Tempo consigliato: ${suggestedWeeks}-${suggestedWeeks + 4} settimane.`);
            return;
        }
    ...
    const handleUpdateWeightDirect = (w: number) => {
        if (isNaN(w) || w <= 0) return;
        const updated = weightLogs.filter(l => l.date !== todayStr);
        updated.push({ id: crypto.randomUUID(), weight: w, date: todayStr });
        saveWeightLogs(updated.sort((a, b) => a.date.localeCompare(b.date)));
        triggerPopup(`⚖️ Peso aggiornato: ${w}kg`);
    };
    ...
    const handleTrackFood = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFoodInput("");
            triggerPopup("✅ AI: Pasto Registrato", data);
        } catch (err) {
            setErrorTracker("Errore nel calcolo istantaneo AI.");
            triggerPopup("❌ Errore AI");
        } finally {
    ...
    useEffect(() => {
        if (!user) return;
        const motivazioni = [
            "🔥 Costanza ferrea. Stai mettendo i mattoni d'acciaio per i primi mesi. Chiudi ogni singolo grammo oggi.",
    ...
    if (user && weightLogs.length > 0) {
        ...
        if (graphCoords.length > 0) {
            ...
            for (let i = 0; i < graphCoords.length - 1; i++) {
                pointsStr += ` C ${graphCoords[i].x + (graphCoords[i+1].x - graphCoords[i].x)/2} ${graphCoords[i].y}, ${graphCoords[i].x + (graphCoords[i+1].x - graphCoords[i].x)/2} ${graphCoords[i].y}, ${graphCoords[i+1].x} ${graphCoords[i+1].y}`;
            }
                    ...
                    <div className="w-full max-w-md bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-8 space-y-6">
                        ...
                        {onboardingStep === 6 && (
                            <div className="space-y-4">
                                ...
                                {errorTracker && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-xs text-red-400 text-center">{errorTracker}</div>}
                ...
                {activeTab === "tracker" && (
                    <div className="space-y-4">
                        <div className="bg-[#111113] border border-white/[0.04] p-5 rounded-3xl shadow-xl space-y-4">
                            ...
                            {errorTracker && <div className="text-xs text-red-400 font-mono mt-1">{errorTracker}</div>}
                        </div>
